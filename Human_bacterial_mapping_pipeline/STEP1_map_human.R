...
filestomap <- grep(".fastq", list.files(foldertomap), value = T)
...
if(ends == "single"){
  for (sample in filestomap2_unique){
    ...
    if(length(samplelist) == 1){
      ...
      command <- paste0("STAR --runThreadN 8 --outFilterScoreMinOverLread 0.3 --outFilterMatchNminOverLread 0.3 ",
                        "--readFilesCommand zcat --outSAMtype BAM SortedByCoordinate --outReadsUnmapped Fastx --readFilesIn ", R1,
...
command <- paste0("featureCounts -a ", genomeannot, " -T 4 -g gene_name -o ", outname, " ", samplesams)
print(command)
system(command, intern = T)
